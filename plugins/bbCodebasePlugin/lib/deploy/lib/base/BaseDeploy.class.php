...
class BaseDeploy
{
	...
	protected $rsync_path = "rsync --rsh='ssh -p 222'";
	//protected $ssh_path = 'ssh';
	protected $ssh_path = 'ssh -p 222';
	...
	public function __construct(array $options)
	{
		...
		$this->rsync_path		= isset($options['rsync_path']) ? $options['rsync_path'] : $this->rsync_path;
		$this->ssh_path			= isset($options['ssh_path']) ? $options['ssh_path'] : $this->ssh_path;

		if (!$this->auto_init)
			return;

		$this->task->logSection('deploy',  'find timestamps: ');
	...
	protected function log($message, $level = 1)
	{
		...
		$this->task->logSection('deploy', $message);
	...
	protected function prepareRemoteDirectory($remote_host, $remote_dir = null)
	{
		...
		if (!empty($this->data_dirs))
		{
			...
			$cmd = "mkdir -v -m 0777 -p $remote_dir/{$this->data_dir_prefix}/$data_dirs";
	...
	protected function sshExec($remote_host, $command, &$output, &$return, $hide_pattern = '', $hide_replacement = '')
	{
		...
		$this->task->logSection('sshExec', $cmd);
		if ($hide_pattern != '')
	...
	protected function rsyncExec($command, $error_msg = 'Rsync is mislukt')
	{
		...
		$this->task->logSection('rsyncExec',  $command);
