...
/**
 * Base static class for performing query and update operations on the 'menu_i18n' table.
 *
 * 
 *
...
abstract class BaseMenuI18NPeer {
	...
	const CLASS_DEFAULT = 'plugins.zeusCorePlugin.lib.model.MenuI18N';
	...
	public static function doCount(Criteria $criteria, $distinct = false, PropelPDO $con = null)
	{
		...
		{
		  call_user_func($sf_hook, 'BaseMenuI18NPeer', $criteria, $con);
		}
	...
	/**
	 * Method to invalidate the instance pool of all tables related to menu_i18n
	...
	/**
	 * Method perform an INSERT on the database, given a MenuI18N or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseMenuI18NPeer:doInsert:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseMenuI18NPeer', $values, $con))
      {
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseMenuI18NPeer:doInsert:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseMenuI18NPeer', $values, $con, $pk);
    }
	...
	/**
	 * Method perform an UPDATE on the database, given a MenuI18N or Criteria object.
	 *
	...
	{
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseMenuI18NPeer:doUpdate:pre') as $sf_hook)
    {
      if (false !== $sf_hook_retval = call_user_func($sf_hook, 'BaseMenuI18NPeer', $values, $con))
      {
        return $sf_hook_retval;
    ...
    // symfony_behaviors behavior
    foreach (sfMixer::getCallables('BaseMenuI18NPeer:doUpdate:post') as $sf_hook)
    {
      call_user_func($sf_hook, 'BaseMenuI18NPeer', $values, $con, $ret);
    }
	...
	/**
	 * Method to DELETE all rows from the menu_i18n table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	/**
	 * Method perform a DELETE on the database, given a MenuI18N or Criteria object OR a primary key value.
	 *
	...
	static private function getMixerPreSelectHook($method)
	{
	  ...
	  {
	    return sprintf('BaseMenuI18NPeer:%s:%1$s', 'Count' == $match[1] ? 'doCount' : $match[0]);
	  }
