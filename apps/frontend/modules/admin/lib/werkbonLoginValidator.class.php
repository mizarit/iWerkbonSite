...
class werkbonLoginValidator {
  ...
  public static function execute ($validator, $value, $arguments) {
  	...
  	$c = new Criteria;
  	$c->add(CredentialsPeer::USERNAME, $username);
  	$c->add(CredentialsPeer::TYPE, 'admin');
  	$c->add(CredentialsPeer::ACTIVE, 1);
  	$users = CredentialsPeer::doSelect($c);
  	$user = false;
  	if ($users) {
  	...
  	if ($user) {
  	  ...
  	  */
      $c->clear();
      $c->add(AdministratorPeer::CREDENTIALS_ID, $user->getId());
      $administrator = AdministratorPeer::doSelectOne($c);
      if ($administrator) {
        sfContext::getInstance()->getUser()->setAttribute('username', $user->getUsername());
        sfContext::getInstance()->getUser()->setAttribute('usertitle', $administrator->getTitle());
        sfContext::getInstance()->getUser()->setAttribute('companyid', $administrator->getCompanyId());
